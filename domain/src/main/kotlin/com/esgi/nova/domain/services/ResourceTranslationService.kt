...
class ResourceTranslationService @Inject constructor(
    ...
    override fun createWithCodes(resourceTranslation: ResourceTranslationLanguageCodesCmdDto): ResourceTranslationDto? {
        languageService.getOneByCodes(resourceTranslation.languageCodes)?.let { l ->
            return resourceTranslationPersistence.create(
                ResourceTranslationLanguageIdCmdDto(
                    resourceTranslation.name,
                    resourceTranslation.resourceId,
                    l.id
                )
            )
        }
    ...
    override fun getOneWithCodes(resourceId: UUID, languageCodes: String): ResourceTranslationDto? {
        languageService.getOneByCodes(languageCodes)?.let { l ->
            return resourceTranslationPersistence.getOne(ResourceTranslationKey(resourceId, l.id))
        }
        return null
    }
    ...
    override fun getOne(id: ResourceTranslationKey): ResourceTranslationDto? = resourceTranslationPersistence.getOne(id)
    ...
    override fun getOneOrDefault(id: ResourceTranslationKey): ResourceTranslationDto? {
